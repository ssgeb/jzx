...
子智能体不再硬编码在 `deep_agent.py`，而是由 `app/loader.py` 从 `python_assistant_service/config/subagents/` 目录加载。`agent_config.py` 只定义配置模型与安全规则，`loader.py` 负责扫描文件、拒绝危险 YAML、合并目录、计算内容摘要和热加载缓存。每个子智能体拥有一个以自身 `name` 命名的 YAML，并独立声明：

- `id`、`name`、是否启用和中文职责说明；
- `responsibilities`：业务责任与禁止越界的范围；
- `skills`：引用 YAML 中受版本控制的可信业务 Skill 指令；
- `tools`：只允许引用 Python `TRUSTED_TOOL_BINDINGS` 已注册的只读工具，并为每个工具显式配置 `human_intervention` 人工介入策略。

当前包含 `detection-specialist.yaml`、`resource-specialist.yaml`、`report-specialist.yaml` 和 `ops-specialist.yaml`。加载器对目录内全部 `*.yaml` 计算联合内容摘要，任一文件变化后，下一次请求都会重新合并并校验整个目录，无需重启 Python 服务。每个文件必须定义自己引用的 Skill 和工具；相同定义可以跨文件复用，但内容必须完全一致。

YAML 使用 Pydantic 严格校验，拒绝未知字段、重复键、锚点、别名、文件名与 Agent 名称不一致、冲突定义、未定义 Skill、未注册工具、伪造工具绑定以及所有子智能体均禁用的配置。配置无效时健康状态显示 `SUBAGENT_CONFIG_INVALID`，查询回到确定性 LangGraph。
...
每个 YAML 工具都必须声明以下安全字段：

- `required`：是否必须由用户确认后才能调用；
- `timing`：当前只允许 `before_tool`，表示真正调用 Java 工具之前暂停；
- `risk_level`：风险级别，可取 `low`、`medium` 或 `high`；高风险工具不能关闭人工介入；
- `approval_message`：需要确认时展示给用户的中文风险说明。

当前检测、资源和报表工具是租户范围内的普通只读查询，配置为低风险自动执行；运维工具 `query_ops` 可能返回服务健康、消息链路和故障诊断信息，因此配置为高风险、调用前人工确认。子 Agent 准备调用它时，Python 不会请求 Java，而是返回 `PENDING_ACTION`，由 Java 将动作和 Checkpoint 持久化到 MySQL。用户确认后，Python 会再次核对动作编号、工具是否仍存在、YAML 工具定义摘要和人工介入策略，全部一致才调用一次工具；拒绝确认或配置在等待期间发生变化时均不执行。

这里复用项目已有的 Java/MySQL 人工确认链路，而没有只依赖 Deep Agents 进程内中断。这样即使确认跨越多个 HTTP 请求、Python 容器重启或实例切换，待确认状态仍可恢复并接受审计。

Docker Compose 将整个配置目录以只读方式挂载到 `/app/config/subagents/`。修改、增加或删除宿主机 YAML 后，容器内下一次请求即可重新加载；只读挂载同时防止 Agent 或容器进程反向修改配置。

这里的 YAML Skill 是随项目发布的可信业务指令，不等于通过网络下载、仍处于 `QUARANTINED` 的外部 Skill。隔离 Skill 不会被 YAML 名称引用自动激活。

业务回答必须至少包含一次成功、非空的 Java 工具调用证据；模型绕过工具直接回答、工具异常或返回空内容时，质量门会拒绝该结果并降级到确定性 LangGraph。普通查询的 Checkpoint 只记录 Agent、工具名和问题摘要哈希，不保存完整工具响应；等待人工确认时会额外保存恢复调用所需的问题、工具定义摘要和动作编号，但仍不保存工具响应。
