...
检测、资源、报表和运维子 Agent 已从 Python 硬编码改为受信 YAML 配置。每个子 Agent 在 `subagents.yaml` 中声明自己的 `id`、名称、启停状态、职责、Skills 和工具；Python 加载器验证后，才把启用的子 Agent 连接到 Harness 主 Agent。

~~~text
修改受版本控制的 subagents.yaml
        │
        ▼
┌──────────────────────────────────────────────────────────────┐
│  步骤一：读取并检测配置变化                                  │
│                                                              │
│  每次请求计算 YAML 的 SHA-256 内容摘要                       │
│  摘要未变化 → 复用已校验配置；摘要变化 → 重新解析           │
└──────────────────────────┬───────────────────────────────────┘
                           ▼
┌──────────────────────────────────────────────────────────────┐
│  步骤二：严格配置校验                                        │
│                                                              │
│  拒绝未知字段、重复键、YAML 锚点/别名、重复 Agent           │
│  校验职责、Skill 引用、工具引用和至少一个启用 Agent          │
└──────────────────────────┬───────────────────────────────────┘
                           ▼
┌──────────────────────────────────────────────────────────────┐
│  步骤三：代码工具白名单二次授权                              │
...
| `tools` | `target_agent`、`operation`、`description` | 声明工具用途，但不能创建代码中不存在的工具 |
| `subagents` | `id`、`name`、`enabled` | 定义子 Agent 身份和是否连接主 Agent |
| `subagents.responsibilities` | 多条职责 | 明确负责范围、缺参处理和禁止越界事项 |
| `subagents.skills` | Skill 名称列表 | 把所需业务指令注入该子 Agent 的系统提示词 |
| `subagents.tools` | 工具名称列表 | 给该子 Agent 分配最小权限工具集 |

所谓“弹性加载”包括三点：修改职责或 Skill 指令后，下一次请求按内容摘要热加载；把 `enabled` 改为 `false` 可从主 Agent 委派目录摘除该子 Agent；增加新的子 Agent 可以复用现有受信 Skill 与工具。真实工具仍不能仅靠 YAML 创建，必须先进入 Python 工具白名单并通过 Java 内部接口实现，避免配置文件变成任意代码执行入口。
...
- python_assistant_service/tests/test_agent_config.py：验证 YAML 职责、Skills、工具解析、内容变化热加载、新子 Agent 复用受信工具、工具代码白名单、重复键、锚点/别名和至少一个启用子 Agent。
