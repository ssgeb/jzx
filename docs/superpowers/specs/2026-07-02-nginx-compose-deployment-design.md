...
如果本机存在可用的 Docker Compose 和已启动的 Docker Engine，再执行 `docker compose -f compose.nginx.yml config` 和容器级冒烟测试。当前机器只有 Docker CLI，Compose 插件不可用且 Docker Engine 未启动，因此仓库级契约测试是本轮必需验证，容器运行验证需在 Docker Desktop 与 Compose 可用后执行。
