...
| 亮点 | 一句话说明 | 详细讲解 |
| --- | --- | --- |
| Hermes 多 Agent 与记忆 | StateGraph 编排专业 Agent，并融合 Checkpoint、Mem0 和 RAG | [进入文档](project-highlights/01-Hermes多Agent编排与分层记忆.md) |
| 智能业务助手 | 用自然语言、SSE 和业务卡片替代菜单检索与多页面表单 | [进入文档](project-highlights/02-智能对话助手与业务升级.md) |
| Kafka 图片检测 | OSS 直传、Kafka 调度、Python ONNX Worker 和幂等结果回传 | [进入文档](project-highlights/03-Kafka异步图片检测.md) |
| 模型与质量追溯 | 模型准入、指标治理、人工复核、处置、返工和工单追溯 | [进入文档](project-highlights/04-ONNX模型治理与质量追溯.md) |
| Nginx 容器部署 | 静态 Web、API 反代、双实例负载均衡和 SSE 无缓冲代理 | [进入文档](project-highlights/05-Nginx与DockerCompose部署.md) |
