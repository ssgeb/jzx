...
/**
 * 资源管理 Agent 节点，包装 ResourceAgentService。
 */
@Component
@RequiredArgsConstructor
public class ResourceAgentNode implements Node {
    ...
    public AgentState execute(AgentState state) {
        ...
        if (Boolean.TRUE.equals(confirmed)) {
            result = resourceAgentService.executeConfirmedAction(prompt, username);
        } else {
            result = resourceAgentService.answer(enrichPromptWithRagContext(prompt, ragContext), username, tokenConsumer);
        }
