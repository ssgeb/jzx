-- Demo data for newly added industrial inspection features.
...
SET `status` = 'PUBLISHED',
    ...
    `validation_message` = '演示数据：ONNX 结构、输入输出维度、类别映射校验通过',
...
SET `status` = 'READY',
    ...
    `validation_status` = 'PASSED',
    `validation_message` = '演示数据：灰度候选模型校验通过',
...
SET `status` = 'READY',
    ...
    `validation_message` = '演示数据：类别顺序与线上配置不一致，暂不允许发布',
...
SELECT 3, 'EVALUATE', 'mlops-admin', '2026-06-10 09:30:00', '演示数据：完成 Q2 验证集评估，达到发布阈值'
WHERE EXISTS (SELECT 1 FROM `model_management` WHERE `model_id` = 3)
  AND NOT EXISTS (
      SELECT 1 FROM `model_operation_log`
      WHERE `model_id` = 3 AND `operation_type` = 'EVALUATE' AND `remark` LIKE '演示数据：完成 Q2%'
  );

INSERT INTO `model_operation_log` (`model_id`, `operation_type`, `operator`, `operation_time`, `remark`)
SELECT 3, 'PUBLISH', 'quality-lead', '2026-06-10 10:00:00', '演示数据：设为默认生产模型'
WHERE EXISTS (SELECT 1 FROM `model_management` WHERE `model_id` = 3)
  AND NOT EXISTS (
      SELECT 1 FROM `model_operation_log`
      WHERE `model_id` = 3 AND `operation_type` = 'PUBLISH' AND `remark` LIKE '演示数据：设为默认%'
  );

INSERT INTO `model_operation_log` (`model_id`, `operation_type`, `operator`, `operation_time`, `remark`)
SELECT 4, 'CANARY', 'mlops-admin', '2026-06-12 15:20:00', '演示数据：候选模型进入 25% 灰度观察'
WHERE EXISTS (SELECT 1 FROM `model_management` WHERE `model_id` = 4)
  AND NOT EXISTS (
      SELECT 1 FROM `model_operation_log`
      WHERE `model_id` = 4 AND `operation_type` = 'CANARY' AND `remark` LIKE '演示数据：候选模型%'
  );
...
SET `online_status` = 'ONLINE',
    ...
    `last_image_key` = 'demo/device/DEV-0001/20260615/latest.jpg',
...
SET `online_status` = 'ONLINE',
    ...
    `last_image_key` = 'demo/device/DEV-0005/20260615/latest.jpg',
...
SET `online_status` = 'OFFLINE',
    ...
    `last_image_key` = 'demo/device/DEV-0021/20260615/failure.jpg',
...
SELECT 'ALERT-DEMO-DEV0021-CAMERA-TIMEOUT', d.`id`, d.`device_code`, d.`device_type`, d.`station_code`, d.`edge_node_id`,
       'CAPTURE_EXCEPTION', 'CRITICAL', '演示数据：相机连续超时，采集端无新图回传',
       '{"cameraStatus":"TIMEOUT","captureStatus":"EXCEPTION","lastImageKey":"demo/device/DEV-0021/20260615/failure.jpg","suggestion":"检查相机供电、网线和边缘节点服务"}',
...
SELECT 'ALERT-DEMO-DEV0005-LOW-LIGHT', d.`id`, d.`device_code`, d.`device_type`, d.`station_code`, d.`edge_node_id`,
       'CAPTURE_EXCEPTION', 'MAJOR', '演示数据：工位光照不足，可能影响缺陷框置信度',
...
SET `model_id` = 3,
    ...
    `assignee` = '李四',
    `assignment_remark` = '演示数据：严重缺陷优先复核',
    `assigned_at` = '2026-06-15 09:35:00',
    `due_at` = '2026-06-15 12:00:00',
    `defect_evidence_json` = '[{"imageKey":"demo/evidence/det_tj_001/original-001.jpg","previewKey":"demo/evidence/det_tj_001/annotated-001.jpg","defectType":"RUST","severity":"MAJOR","confidence":0.934,"bbox":{"x":126,"y":88,"w":72,"h":54},"area":3888,"position":"handle-left-hinge"},{"imageKey":"demo/evidence/det_tj_001/original-002.jpg","previewKey":"demo/evidence/det_tj_001/annotated-002.jpg","defectType":"DEFORMATION","severity":"CRITICAL","confidence":0.912,"bbox":{"x":220,"y":144,"w":96,"h":68},"area":6528,"position":"handle-lock-seat"}]',
    `defect_count` = 2,
...
SET `model_id` = 3,
    ...
    `assignee` = '王五',
    `assignment_remark` = '演示数据：已完成复核，等待处置',
    `assigned_at` = '2026-06-15 08:20:00',
    `due_at` = '2026-06-15 11:00:00',
    `defect_evidence_json` = '[{"imageKey":"demo/evidence/det_tj_002/original-101.jpg","previewKey":"demo/evidence/det_tj_002/annotated-101.jpg","defectType":"MISSING_PART","severity":"CRITICAL","confidence":0.958,"bbox":{"x":172,"y":98,"w":88,"h":80},"area":7040,"position":"handle-pin"}]',
    `defect_count` = 1,
    ...
    `false_positive_count` = 0,
    `review_remark` = '演示数据：确认门把手销轴缺失，需隔离并返工',
    `reviewer` = '王五',
...
SET `model_id` = 3,
    ...
    `assignee` = '赵磊',
    `assignment_remark` = '演示数据：已处置为返工，等待复检',
    `assigned_at` = '2026-06-14 15:30:00',
    `due_at` = '2026-06-15 16:00:00',
    `defect_evidence_json` = '[{"imageKey":"demo/evidence/det_tj_003/original-205.jpg","previewKey":"demo/evidence/det_tj_003/annotated-205.jpg","defectType":"BEND","severity":"MAJOR","confidence":0.887,"bbox":{"x":208,"y":120,"w":65,"h":90},"area":5850,"position":"handle-grip"}]',
    `defect_count` = 1,
...
SET `model_id` = 3,
    ...
    `assignee` = '张强',
    `assignment_remark` = '演示数据：误报样本，已放行',
    `assigned_at` = '2026-06-13 14:00:00',
    `due_at` = '2026-06-14 10:00:00',
    `defect_evidence_json` = '[{"imageKey":"demo/evidence/det_tj_004/original-302.jpg","previewKey":"demo/evidence/det_tj_004/annotated-302.jpg","defectType":"SCRATCH","severity":"MINOR","confidence":0.641,"bbox":{"x":90,"y":72,"w":42,"h":28},"area":1176,"position":"handle-surface"}]',
    `defect_count` = 1,
    ...
    `review_remark` = '演示数据：表面反光导致误检，人工复核为合格',
    `reviewer` = '张强',
    `reviewed_at` = '2026-06-13 15:30:00',
    `disposition_status` = 'DISPOSED',
    `disposition_action` = 'RELEASE',
    `disposition_remark` = '演示数据：误报确认后放行',
    `disposition_operator` = '质检主管',
...
SET `model_id` = 4,
    ...
    `assignee` = '李静',
    `assignment_remark` = '演示数据：返工完成，等待复检',
    `assigned_at` = '2026-06-12 11:00:00',
    `due_at` = '2026-06-15 15:30:00',
    `defect_evidence_json` = '[{"imageKey":"demo/evidence/det_tj_005/original-410.jpg","previewKey":"demo/evidence/det_tj_005/annotated-410.jpg","defectType":"RUST","severity":"MAJOR","confidence":0.902,"bbox":{"x":150,"y":122,"w":74,"h":58},"area":4292,"position":"handle-base"}]',
    `defect_count` = 1,
    `primary_defect_type` = 'RUST',
    `max_defect_severity` = 'MAJOR',
    `review_status` = 'REVIEWED',
